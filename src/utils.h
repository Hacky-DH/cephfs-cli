...
inline std::ostream& log(const char *level){
    ...
    char buf[100];
    std::strftime(buf, 100, "%F %T", std::localtime(&t));
    return get_stream()<<buf<<" ["<<level<<"] ";
}

inline void error(const char* msg, const char* path, int e){
    std::string _msg, _path;
    if(msg == nullptr)
        _msg = "";
    else
        _msg = msg;
    if(path == nullptr)
        ...
        _path = path;
    if(e != 0)
        log("ERROR")<<_msg<<_path<<": ("<<e<<") "<<strerror(e)<<std::endl;
    else
        log("ERROR")<<_msg<<_path<<std::endl;
}
...
inline size_t parse_obj_size(const char * arg){
    size_t base = std::atoi(arg);
    char unit = arg[strlen(arg)-1];
    if(std::isalpha(unit)){
        unit = std::tolower(unit);
        if(unit=='t') base *= 1024*1024*1024*1024l;
        else if(unit=='g') base *= 1024*1024*1024l;
